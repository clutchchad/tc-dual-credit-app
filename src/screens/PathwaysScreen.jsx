...
function PathwayList({ school, onBack }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Color bar */}
      <div style={{ backgroundColor: school.color, height: 64, display: 'flex', alignItems: 'center', paddingLeft: 14, paddingRight: 14, flexShrink: 0 }}>
        {onBack && (
          <button
            onClick={onBack}
...
export default function PathwaysScreen({ onNavigate, tabs }) {
  ...
  return (
    <div className="tc-screen" style={{ width: '100%', height: '100%', background: C.bg, display: 'flex', flexDirection: 'column' }}>
      {isGuest ? (
        browsingSchool ? (
          <PathwayList school={browsingSchool} onBack={() => setBrowsingSchool(null)} />
        ) : (
          <>
        ...
        <PathwayList school={school} onBack={null} />
      )}
